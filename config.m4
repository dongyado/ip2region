...
if test "$PHP_IP2REGION" != "no"; then
  ...
  dnl # --with-ip2region -> check with-path
  dnl SEARCH_PATH="/usr/local /usr"     # you might want to change this
  dnl SEARCH_FOR="/include/ip2region.h"  # you most likely want to change this
  ...
  dnl # --with-ip2region -> add include path
  dnl PHP_ADD_INCLUDE($IP2REGION_DIR/include)

  dnl # --with-ip2region -> check for lib and symbol presence
  ...
  PHP_NEW_EXTENSION(ip2region, ip2region.c, $ext_shared)
fi
